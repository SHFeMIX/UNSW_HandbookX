        ...
        <div class="content">
            <div>
                ...
                <div class="h-[60px] mx-[3px] flex justify-between items-center" v-for="item in searchHistory">
                    <div class="w-[150px] font-normal tracking-[0.1px] leading-[15.4px] text-[11px]"
                        style="font-family: Yinmar">{{item.name}}</div>
                    <div class="flex flex-col items-end" style="font-family: Yinmar">
                        <div class="font-normal tracking-[0.3px] leading-[14px] text-[10px] text-[#96A0B5] mb-[6px]">{{item.code}}</div>
                        <div class="font-normal tracking-[0.3px] leading-[14px] text-[10px] px-[7px] bg-[#FFCC00] h-[14px] rounded-[3px]">{{item.kind}}</div>
                    </div>
                </div>
            </div>
            <div>
...
watch(searchText, async () => {
    if (isRecommendating.value) {
        const data = await fetch('http://127.0.0.1:4523/m2/3791963-0-default/218194114')
        ...
        //         kind: 'Course'
        //     }
        // ].slice(0, searchText.value.length)
        // recommendation.value = await fetchRecommendation(searchText.value)
    }
})
